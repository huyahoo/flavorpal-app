    ...
    <div class="flex-grow flex flex-col">
      <div v-if="isCameraActive" class="flex-grow relative flex items-center justify-center bg-black">
        ...
        <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[80%] aspect-[4/3] sm:w-[70%] sm:max-w-md pointer-events-none">
         ...
         <p v-if="cameraError" class="absolute top-16 left-1/2 -translate-x-1/2 z-10 bg-red-600/80 text-white text-xs px-3 py-1.5 rounded-md shadow-lg" :style="{top: `calc(env(safe-area-inset-top) + 4rem)`}">{{ cameraError }}</p>
      ...
      <div v-if="scanStore.currentStage === 'idle_choice' && !isCameraActive" class="flex flex-col h-full">
        ...
        >
          ...
          <div class="flex justify-around items-center">
            <button @click="attemptStartCameraScanWithZXing" class="flex flex-col items-center text-gray-200 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/10 focus:outline-none focus:bg-white/10" aria-label="Scan Barcode with Camera"> <svg class="w-10 h-10 sm:w-12 sm:h-12 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path></svg> <span class="text-xs font-medium">Scan Barcode</span> </button>
            <button @click="scanStore.prepareBarcodeScan()" class="flex flex-col items-center text-gray-200 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/10 focus:outline-none focus:bg-white/10" aria-label="Enter Barcode Manually"> <svg class="w-10 h-10 sm:w-12 sm:h-12 mb-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path d="M3 11H2V9H3V5C3 3.9 3.9 3 5 3H9V5H5V9H9V11H5V15H9V17H5C3.9 17 3 16.1 3 15V11ZM19 17H15V15H19V11H15V9H19V5H15V3H19C20.1 3 21 3.9 21 5V9H22V11H21V15C21 16.1 20.1 17 19 17ZM13 15H11V9H13V15Z"></path></svg> <span class="text-xs font-medium">Enter Manually</span> </button>
            <button @click="handlePhotoScan" class="flex flex-col items-center text-gray-200 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/10 focus:outline-none focus:bg-white/10" aria-label="Take Photo"> <svg class="w-10 h-10 sm:w-12 sm:h-12 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"></path></svg> <span class="text-xs font-medium">Product Photo</span> </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="scanStore.currentStage === 'inputting_barcode' && !isCameraActive" class="flex flex-col h-full items-center justify-center p-6 bg-gray-700" :style="{ paddingBottom: `calc(env(safe-area-inset-bottom, 0px) + 1.5rem)` }">
          ...
          <form @submit.prevent="submitBarcode" class="w-full max-w-sm">
              ...
              <div class="flex space-x-3">
                  <button type="button" @click="scanStore.resetScanView()" class="flex-1 bg-gray-600 hover:bg-gray-500 text-white font-medium py-2.5 px-6 rounded-lg transition-colors"> Cancel </button>
                  <button type="submit" :disabled="!scanStore.scannedBarcodeValue.trim()" class="flex-1 bg-flavorpal-green hover:bg-flavorpal-green-dark text-white font-medium py-2.5 px-6 rounded-lg transition-colors disabled:opacity-50"> Submit </button>
              </div>
          </form>
      </div>
      ...
      <div v-if="scanStore.currentStage === 'analyzing'" class="absolute inset-0 bg-gray-800/95 backdrop-blur-sm flex flex-col items-center justify-center z-40 p-6">
          <svg class="animate-spin h-12 w-12 text-flavorpal-green mb-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"> <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle> <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path> </svg>
          ...
          <button @click="scanStore.cancelAnalysis()" class="bg-red-600 hover:bg-red-700 text-white font-medium py-2.5 px-6 rounded-lg transition-colors"> Cancel Analysis </button>
      </div>
