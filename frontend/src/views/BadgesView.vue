<template>
  <div class="flex flex-col min-h-full bg-flavorpal-gray-light">
    ...
    <main class="flex-grow p-4 sm:p-6 space-y-5 overflow-y-auto">
      <div v-if="userProfileStore.loading" class="text-center py-10">
      ...
      <div v-else-if="userProfileStore.badges.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg gap-x-5 gap-y-5">
        <BadgeCard
          v-for="badge in userProfileStore.badges"
          :key="badge.id"
          :badge="badge"
      ...
      <p v-if="userProfileStore.error && !userProfileStore.loading" class="text-center text-red-500 py-4">
...
onMounted(() => {
  // Load badges data from userProfileStore
